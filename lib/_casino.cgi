...
sub read_member {
	...
	$this_title = $stage eq 'ドッペル' ? "$p_name 賭けｺｲﾝ<b>$bet</b>枚"
...
sub give_coin {
	...
	$com.=qq|<span class="st_down">ｶｼﾞﾉｺｲﾝを <b>$give_coin</b>枚 支払いました</span>|;
}
...
sub reload_member {
	...
	for my $name (@members) {
		...
		if ($p{coin} <= 0) {
			$npc_com .= qq|<span class="die">$nameはｺｲﾝがなくなった！</span><br />|;
